...
|----|------|
| #2 | 城市 geocode 修复 |
| #3 | 冷启动检索阶梯（餐饮+场所）+ 空池安全网 |
| #4 | 时长尊重 + 时间校验 + 前端时长控件 |
| #5 | 重规划状态中文文案 |
| #6 | 高德链接 + 去预订 + 分享导出（邮箱 / 图标） |
| #7 | Docker 化 + GitHub Actions CI |
| #8 | 多日方案质量（跨天去重 / 方案多样 / 日期分隔符） |
| #9 | LongCat（美团）provider |
| #10 | 多日生成提速（并发单方案 + 请求超时 + 软/硬重试拆分） |
| #11 | 行程清单（完成/预订勾选 + 一键预订 + 复制/PDF/邮件分享） |
| #12 | 全国任意城市可用（geocode + 周边搜索） |
| #13 | 提交材料（设计文档 + README 首页） |
...
Phase 9  集成测试                        ✅ 完成（手动 + 131 单测）
```

> Phase 9 之后的增强均已完成并合并：性能提速、行程清单、全国城市可用、Docker/CI、LongCat。
...
**修复**：`amap_http.py` 新增 `geocode_city()`——先查 15 城缓存，未命中调高德 Geocoding API 动态解析并缓存；`nodes.py` 改用之，删除 hardcoded dict。境外城市（如 Zurich）受限于高德 API 覆盖范围，属已知限制（数据源决定，非缺陷）。
