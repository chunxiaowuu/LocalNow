...
> **说明** —— 本文是按时间顺序的开发记录。检索层从 RAG/ChromaDB 重构为直接调用地图 API（见 Step 9）；最初的 RAG 模块已**删除**。当前数据流见 [architecture.zh-CN.md](architecture.zh-CN.md) 与 [design.zh-CN.md](design.zh-CN.md)。
...
每个场景过滤后约 15-20 条有效候选，满足 Agent 规划和排序的需求。
...
手工数据放在合并数组的前面，保证关键场景记录在检索中优先出现。
...
| 文件 | 职责 |
|------|------|
| `tools/amap_http.py` | 地图客户端：`geocode_city`、周边 POI 搜索、映射为 `Venue`/`Restaurant`；无 API Key 或调用失败时降级到本地 mock JSON |
| `tools/geo.py` | haversine 距离、贪心地理聚类（纯函数） |
| `tools/travel.py` | 游玩时长常量、交通时间估算、聚类半径（纯函数） |
| `tools/links.py` | 由名称 + 坐标拼接地图/预订跳转链接（不让 LLM 编造） |
| `tools/notification.py` | 渲染可分享的行程摘要 |
...
**本次迁移一并删除**：RAG 数据层（`tools/store.py` 的 ChromaDB 索引、`tools/search.py` 两阶检索、`tools/availability.py` 基于 store ID 的检查，以及独立的 `tools/booking.py`）已成为死代码，已删除。可用性改为基于候选数据内联检查，`execute_bookings` 直接构造 `BookingResult`。
