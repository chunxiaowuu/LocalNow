...
- 技术：CoT prompt 引导约束推理，with_structured_output 强制结构化输出
...
LangChain `with_structured_output` + 业界标准的 validation-retry loop（思路参考 instructor-ai）：
...
> **架构演进说明**：初始版本使用 ChromaDB in-memory 做向量语义检索（RAG），后改为直接调用地图 REST API。真实数据覆盖全国 POI，比 80 条 mock 数据更能体现实际可用性，真实 API 集成也比本地向量检索更能体现工程能力。最初的 RAG 模块（`tools/store.py`、`tools/search.py`）已删除。
...
用户提具体冷门诉求（"某家拉面馆""莫奈特展"）时，精确关键词常召回为空。解法在**召回侧**做语义降级，而非过滤候选结果：
...
| 结构化输出 | with_structured_output + Pydantic v2 | LLM 输出验证重试 |
