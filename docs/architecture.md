...
> **架构演进说明**：初始版本使用 ChromaDB in-memory 做向量语义检索（RAG），后改为直接调用高德地图 REST API。真实数据覆盖全国 POI，比 80 条 mock 数据更能体现实际可用性，且对 LLM 求职作品集而言，真实 API 集成的技术含量高于本地向量检索。`tools/store.py` 和 `tools/search.py` 已成为遗留代码，仅 `booking.py`（mock 预订）还引用 store。
