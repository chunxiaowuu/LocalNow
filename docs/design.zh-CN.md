...
2. **候选召回 `search_candidates`**
   `geocode 地名→坐标` + **周边搜索**召回真实场所/餐厅（全国任意城市/区县/景区可用）；haversine 距离 + 游玩时长 + 人均预算**硬过滤**；按评分（热度代理）排序；按天数做地理聚类。
   *冷启动语义降级*：冷门诉求（"某家拉面馆"）由 LLM 产出"具体→宽泛"检索词阶梯，逐级召回直到有结果，降级到相近的人气候选并向用户说明。
...
**技术栈**：Python · FastAPI · LangGraph · 地图 REST API · Next.js · Docker + GitHub Actions CI；多模型 Provider 抽象（Gemini / LongCat / OpenAI / DeepSeek / Ollama，改 `.env` 即切换）。
