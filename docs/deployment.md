...
超额返回 `429` + 中文提示。密钥仅存在后端环境变量，永不下发浏览器。
...
> ⚠️ **不要**配置 `GOOGLE_API_KEY` / `ANTHROPIC_API_KEY` / `OPENAI_API_KEY`——线上只用美团 key。
...
仍按 `README.md`：`LLM_PROVIDER` 可任意（如 `gemini`，更快），不设 OAuth/限流相关变量时登录按钮隐藏、限流按匿名（每天 1 次）。Docker 一键：`docker compose up --build`。
